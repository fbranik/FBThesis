...
//        check fail for 32 nodes 512 640 procs and 16 nodes 320 procs
      }
    uStart = &U[0][0];
  }
...
#ifdef COMP_BOUND
    sprintf(s, "results/NewWorkingsetSizes/%dnodes/CompBoundResults_%s_%dx%d_%dn.out", numNodes,
            workingSetSize, grid[0], grid[1], numNodes);
#elif MEM_BOUND
    sprintf(s, "results/NewWorkingsetSizes/%dnodes/MemBoundResults_%s_%dx%d_%dn.out", numNodes,
            workingSetSize, grid[0], grid[1], numNodes);
#endif
...
#ifdef SOCKET_BARRIER
    fprintf(f, "Jacobi (Socket Barrier) X %d Y %d Px %d Py %d Iter %d\n"
               "Working Set Size %s\n"
               "Extra Iterations %d\n"
               "Num. Dummy MSGs %d\n"
               "MSG Size %d\n"
...
#else
    fprintf(f, "Jacobi X %d Y %d Px %d Py %d Iter %d\n"
               "Working Set Size %s\n"
               "Extra Iterations %d\n"
               "Num. Dummy MSGs %d\n"
               "MSG Size %d\n"
